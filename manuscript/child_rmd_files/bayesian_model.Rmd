...
memory representation of a text is a representation of it semantic content.  We
...
and many others.  In other words, we can describe any given text by identifying
...
possibly large set of semantically inter-related words.  Each discourse topic,
therefore, can be approximately identified by a cluster of semantically
inter-related words.  Whenever we read or hear a text, we infer its discourse
...
for any particular text, denoted by $\text{j}$, we can then infer, 
again via Bayes's rule,
$$
\Prob{\pi_j \given \textj{j}},
$$
where $\pi_j$ is a probability distribution over the $K$ 
...
$$
\Prob{w \given \textrm{text}_j} = 
\int\! \Prob{w \given \pi_j} \Prob{\pi_j \given \text{j}}\ d\pi_j,
$$
which gives the probability distribution over the words that 
are predicted or implied by the $\pi_j$ representation of $\text{j}$.
...
V\}$, or $\data = \{w_{j1}, w_{j2} \ldots w_{ji} \ldots w_{jn_j}\}_{j=1}^J$.
...
English words[^standard_dictionary], there was a total vocabulary of 49,324 word types in the corpus.
...
$m$, which we'll denote by $\bar{\phi}$, $\bar{a}$, and $\bar{m}$, respectively,
...
are show in Figure \ref{fig:wordclouds}. In each case, the posterior predictive
...
words will and will not, whether veridically or falsely, from any given text.
