...
The behavioural experiments show that after reading everday texts under normal
reading conditions, participants nontheless have relatively high false positive
...
it has a high probability of being remembered, either falsely or veridically,
after having read that text. Compared to these associative models of text memory, 
...
models of text memory, the Coccurrence model had consistently better performance
than the Associative model. Recall that these two models differ from one another
solely in terms of their definition of the association between two words, with
the Coccurrence model defining association in terms of statistical cooccurrence
within a corpus, and the Associative model defining association in terms of word
association norm data. One relatively simple explanation for the difference in
the predictive accuracy of these two models is simply a matter of sparse or
missing data. Although the association norms that we have used is a very large
example of its kind, it nonetheless only provides associates for a set of
approximately 10,000 word types. Moreover, obtaining reliable data for low
probability associates of any given word generally requires prohibitively large
numbers of responses. We endeavoured to minimize the effect of data sparsity
arising from the word association norms by only using texts where most words
were in word association norm stimulus set. Nonetheless, this may not be
sufficient to make the predictions based on these norms comparable to those 
based on the cooccurrence statistics, which are obtained from a relatively
large corpus of 80 million words. If this explanation of the difference 
is correct, there may be nothing theoretically interesting about the relative 
performance of the Cooccurrence and Associative models.
...
and spreading account of episodic memory is the dominant account of the \drm
...
<!-- . This involves describing discourse topics in terms of  -->


<!-- forming a semantic representation of what the text is about. -->




<!-- When applied to memory for -->
<!-- text, the problem being faced crucially involves forming a semantic representation of what the text is about -->


<!-- as that outlined in Figure -->



<!-- according the widely held account of the problem being crucially involves  -->



<!-- the problem we are faced with involves -->
...
<!-- For language comprehension, it is uncontroversial to say that  problem we are faced with is forming a semantic representation of what the text is about -->

<!-- As we have described, it is widely held and unconstroversial that -->
<!-- the problem being faced when  -->

<!-- It is uncontroversial to say that whenever we hear or read spoken or written language, -->
<!-- we are faced with the a major computational problem we are faced with is forming a semantic representation of what the text is about -->
...
<!-- anderson1991reflections -->

<!-- outlines a rational account of episodic memory that describes the principal computational problem being faced as the -->
...
<!-- sense of ; see also @shagrir_on_marr) or *rational* (in the -->
<!-- sense of ) approaches to episodic memory in -->
<!-- general, and memory for text in particular. In general, computational or -->
<!-- rational models are premised on the hypothesis that all aspects of cognition can -->
<!-- be understood as adaptive solutions to computational problems posed by the -->
<!-- environment. -->


<!-- Thus, for example, @anderson:adaptive outlines a rational account of episodic memory that describes the principal computational problem being faced as the -->
<!-- storage of information currently being experienced that may become relevant at -->
<!-- some point in the future.  -->

<!-- visual object recognition -->
<!-- can be understood as a solution to the problem of determining the object in the -->
<!-- world that is the cause of a given stimulus pattern on the retina . -->
<!-- Likewise,  -->

<!-- Applied to the problem of memory for texts, determining precisely -->
<!-- what information in a text may be relevant requires a probabilistic model of -->
<!-- the general statistical patterns across spoken and written language. -->
<!-- Representing the information in a given text in terms of these patterns would -->
<!-- identify precisely those that are likely to become relevant again at some point -->
<!-- in the future.  -->
