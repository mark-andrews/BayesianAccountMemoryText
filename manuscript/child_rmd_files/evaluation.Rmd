...
University and elsewhere, with the only restriction being that they be native
English speakers.
...
Given that neighbouring paragraph will usually be semantically related to the
...
count in the corpus as a whole, $n_i$ is number of documents in the corpus where
...
Participants were instructed to read this text at their normal reading.  The
text stayed on screen for a maximum of 90 seconds, but after 45 seconds,
participants were able to move on the next screen if they so wished. On the
...
is that the former memories are ultimately based on the meaning of text, while
...
<!-- Summarize the recognition test results -->
...
with the false positive rate being `r 100*round(false_positive_rate, 2)`% 
...
### Recognition memory data: Exploratory analyses
...
### Recognition memory data: Multilevel logistic regression analyses
...
### Recall memory data: Exploratory analyses 
...
$$
L^{\text{Bayesian}}_{jk} = \sum_{\{i\in\mathcal{S}_{jk}\}} \log\left(\psibayes_{(k, y_i)}\right),
$$
where $\mathcal{S}_{jk} \triangleq \{i \colon s_i = j \land t_i = k\}$
is the set of all trials where $s_i = j$ and $t_i=k$. 
...
### Recall memory data: Multilevel logistic regression analyses
